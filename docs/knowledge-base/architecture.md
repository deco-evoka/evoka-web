...
Los nombres autorizados exclusivos de tarjeta se resuelven por ID y locale mediante `catalogProductNames` en `src/i18n/ui.ts`. Se aplican también al nombre de su vista ampliada y búsqueda, sin cambiar los nombres de origen que determinan el orden y las URLs de las fichas. Las descripciones cortas siguen procediendo de `catalog-short-descriptions.ts`, sin modificaciones. En multicolumna, las tarjetas comparten cinco filas de contenido mediante CSS subgrid; hasta 440 px conservan el flujo natural.
